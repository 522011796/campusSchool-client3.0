    ...
    <div class="content-block" :style="{height: divHeight20.height}" style="overflow-y: auto">
      ...
      <div class="content-botton-block text-center">
        <el-row style="margin-top: 8px" :gutter="16">
          ...
          <el-col :span="12">
            <div class="text-center padding-lr-10">
              <van-button round size="small" type="info" block @click="saveForm(2)">{{$t("提交")}}</van-button>
            </div>
          </el-col>
        </el-row>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      initHt(){
        ...
        this.$axios.get(common.ht_list, {params: params}).then(res => {
          if (res.data.data){
            let array = [];
            for (let i = 0; i < res.data.data.length; i++){
              array.push({
      ...
      initTag(){
        ...
        this.$axios.get(common.tag_list, {params: params}).then(res => {
          if (res.data.data){
            let array = [];
            for (let i = 0; i < res.data.data.length; i++){
              array.push({
